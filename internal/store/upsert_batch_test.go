...
func TestUpsertBatch_PopulatesEnTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("en", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "en").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "en")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count en: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("en count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
